...
/*void __interrupt(high_priority) HighISR() { 
    ...
    if (PIE4bits.TX4IE && PIR4bits.TX4IF) { // if interrupt and flag are set to the same
        if (isDataInTxBuf()) { // if data is in the Tx buffer
            TX4REG = getCharFromTxBuf(); // send the info from the Tx buffer to the Tx register
        } 
        else {
            PIE4bits.TX4IE = 0; // switch the interrupt off
    }
    }
}*/